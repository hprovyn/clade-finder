...
def decorateSNPProductsMultiLevels(obj, tbSNPclades):
    snps = []
    for snp in obj["phyloeq"]:
        if obj["phyloeq"][snp]["call"] == "?":            
            snps.append(snp)
    ...
    if "downstream" in obj:
        for child in obj["downstream"]:
            decorateSNPProductsMultiLevels(child, tbSNPclades)
    
def decorateSNPProductsAutoPanel(obj, tbSNPclades):
    products = getSNPProductsTabix(obj["phyloeq"].keys(), tbSNPclades)
    todel = []
    for snp in obj["phyloeq"]:
        if snp not in products:
            todel.append(snp)
        else:
            obj["phyloeq"][snp]["product"] = products[snp]
    thedelobj = obj["phyloeq"]
    for snp in todel:
        del thedelobj[snp]

    if "downstream" in obj:
        for child in obj["downstream"]:
            decorateSNPProductsAutoPanel(child, tbSNPclades)
...
def decorateJSONObjectMultiLevels(params, clade, levels, score, positives, negatives, tbCladeSNPs, tbSNPClades, hierarchy, snpPanelConfigFile, conflicts, warning):
    theobj = {}
    clade = clade.replace("*","")
    theobj["clade"] = clade
    if "downstream" in params:
        theobj["downstream"] = getDownstreamSNPsJSONObjectMultiLevels(clade, positives, negatives, conflicts, tbCladeSNPs, levels, False)
    if "phyloeq" in params:
        theobj["phyloeq"] = getCladeSNPStatusJSONObject(clade, positives, negatives, conflicts, tbCladeSNPs)
    if "score" in params:
        theobj["score"] = score
    if "products" in params:
        decorateSNPProductsMultiLevels(theobj, tbSNPClades)
    if "panels" in params and hierarchy is not None and snpPanelConfigFile is not None:
        panels = getPanelArray(clade, snpPanelConfigFile, tbCladeSNPs, hierarchy, negatives)
        if len(panels) > 0:
            theobj["panels"] = panels
...
def decorateAutoPanelJSONObject(params, clade, score, tbCladeSNPs, tbSNPClades, hierarchy, snpPanelConfigFile, warning, levels):
    theobj = {}
    clade = clade.replace("*","")
    theobj["clade"] = clade
    if "downstream" in params:
        ...
        negatives = []
        conflicts = []
        theobj["downstream"] = getDownstreamSNPsJSONObjectMultiLevels(clade, positives, negatives, conflicts, tbCladeSNPs, levels, True)
    if "phyloeq" in params:
        theobj["phyloeq"] = getAutoPanelCladeSNPsJSONObject(clade, tbCladeSNPs)
    if "score" in params:
        theobj["score"] = score
    if "products" in params:
        decorateSNPProductsAutoPanel(theobj, tbSNPClades)
    if warning:
        theobj["warning"] = warning
    if "phyloeq" in params:
        return decodeTabixSNPs(theobj)
...
def getJSONForClade(params, clade, levels, positives, negatives, tbCladeSNPsFile, tbSNPcladesFile):
    return json.dumps(getJSONObjectForClade(params, clade, levels, positives, negatives, tbCladeSNPsFile, tbSNPcladesFile))

def getJSONObjectForClade(params, clades, levels, positives, negatives, tbCladeSNPsFile, tbSNPcladesFile):
    ...
    for clade in clades:
        if (levels == 1):
            theObjects.append(decorateJSONObject(params, clade, 0, uniqPositives, uniqNegatives, tbCladeSNPs, tbSNPclades, None, None, conflicting, warning))
        else:
            theObjects.append(decorateJSONObjectMultiLevels(params, clade, levels, 0, uniqPositives, uniqNegatives, tbCladeSNPs, tbSNPclades, None, None, conflicting, warning))
    return theObjects
    
def getJSONForAutoPanel(params, clades, levels, positives, negatives, tbCladeSNPsFile, tbSNPcladesFile):
    return json.dumps(getJSONObjectForAutoPanel(params, clades, levels, positives, negatives, tbCladeSNPsFile, tbSNPcladesFile))

def getJSONObjectForAutoPanel(params, clades, levels, positives, negatives, tbCladeSNPsFile, tbSNPcladesFile):
    tbSNPclades = tabix.open(tbSNPcladesFile)
    tbCladeSNPs = tabix.open(tbCladeSNPsFile)
    warning = None
    theObjects = []
    for clade in clades:
        theObjects.append(decorateAutoPanelJSONObject(params, clade, 0, tbCladeSNPs, tbSNPclades, None, None, warning, levels))
    return theObjects
...
def getAutoPanelCladeSNPsJSONObject(clade, tbCladeSNPs):
...
def getAutoPanelDownstreamSNPsJSONObject(clade, tbCladeSNPs, levels):
    children = getChildrenTabix(clade, tbCladeSNPs)
    downstreamNodes = []
    for child in children:
        downstreamNode = {"clade": child}
        
        downstreamNode["phyloeq"] = getAutoPanelCladeSNPsJSONObject(child, tbCladeSNPs)
        grandChildren = getChildrenTabix(child, tbCladeSNPs)
        if len(grandChildren) > 0:
            if levels > 2:
                downstreamNode["downstream"] = getAutoPanelDownstreamSNPsJSONObject(child, conflicts, tbCladeSNPs, levels - 1)
            else:
                downstreamNode["children"] = len(grandChildren)
        downstreamNodes.append(downstreamNode)
    return downstreamNodes

def getDownstreamSNPsJSONObjectMultiLevels(clade, positives, negatives, conflicts, tbCladeSNPs, levels, isAutoPanel):
    children = getChildrenTabix(clade, tbCladeSNPs)
    downstreamNodes = []
    for child in children:
        downstreamNode = {"clade": child}
        ...
        if len(grandChildren) > 0:
            if levels > 2:
                downstreamNode["downstream"] = getDownstreamSNPsJSONObjectMultiLevels(child, positives, negatives, conflicts, tbCladeSNPs, levels - 1, isAutoPanel)
            else:
                downstreamNode["children"] = len(grandChildren)
        downstreamNodes.append(downstreamNode)
    return downstreamNodes
