...
def findClade(positives, negatives, tbCladeSNPsFile, tbSNPcladesFile, snpPanelConfigFile):
    ...
    if len(b) > 0:
        ...
        for recommendedPanel in panelsEqualToPrediction:
            count = count + 1
            html = html + str(count) + ". " + panels[recommendedPanel] + "<br><i>Predicted " + res[1] + " is the panel root. This panel is applicable and will definitely provide higher resolution</i><br><br>"
        
        if count == 0:
            for recommendedPanel in sortPanelRootsUpstream(panelRootsUpstreamPrediction, res[1], hierarchy):
                count = count + 1
                html = html + str(count) + ". " + panels[recommendedPanel] + "<br><i>Predicted " + res[1] + " is downstream of the panel root. This panel is applicable and may provide higher resolution if it tests subclades below " + res[1] + "</i><br><br>"
        for recommendedPanel in panelsDownstreamPrediction:
            count = count + 1
            html = html + str(count) + ". " + panels[recommendedPanel] + "<br><i>Panel may be applicable, but not guaranteed until confirmed positive for panel root SNP</i><br><br>"
